...
- 2026-09-19 (P122 in tree, UNCOMMITTED): C++ face `blinky/ble_fw/ble_cpp_face.cpp` (xpack g++, same link script, bit-identical rebuild; one svc#imm per static method — shared r3/ip dispatcher miscompiles; `nrf_ble_cpp_face_markers`, markers `P:*`, 2nd-run clean) + TypeScript face `demo/parts/ble_lang/ts_lang_face.mts` (strict types, BLE + RADIO, `npm run test:ts` folded into `test:wasm`). Suite 226 single green (= 116 cpu incl. 20 fw proofs); `npm run test:wasm` (handshake + smoke + mpy + ts) all green; browser 16/16. NEXT: commit per approval.
