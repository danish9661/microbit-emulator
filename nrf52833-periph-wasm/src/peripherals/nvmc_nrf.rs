...
/// Read-disable is STORED and queryable (`acl_read_blocked_at`) but
/// CPU-read enforcement needs a memory-layer hook (TODO — mem.rs lives
/// under the AGENTS.md cpu rule, so reads behave as unprotected today).
/// No SPU exists on nRF52833 (70 SVD peripherals, none named SPU —
/// SPU is an nRF53/nRF91 part); "ACL/SPU" rows elsewhere mean ACL.
pub struct Nvmc {
    pub config: u32,
...
impl Nvmc {
    ...
    /// True when `erase_len` bytes at `base` hit any write-protected
    /// ACL region (PERM bit 1 set, SIZE nonzero).
...
/// SIZE nonzero). Enforcement hook for a future memory-layer gate;
/// today the query is informational (reads behave unprotected — the
/// mem.rs hook needs the cpu-rule exception, TODO above).
