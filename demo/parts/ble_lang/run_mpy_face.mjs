// Runs demo/parts/ble_lang/mpy_ble_face.py logic from Node against the
// built wasm pkg (CPython also runs the .py directly). Exits nonzero
// on any FAIL — same contract as handshake.mjs.
...
check(svc(0x60, 0, 0) === 0, "mpy enable");
cpu.mem_write(0x20001200, [1, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66]);
check(svc(0x8c, 0x20001200, 0) === 0, "mpy connect stages");
const job = wasm.ble_take_job();
check(job.length > 0 && job[0] === 1, "mpy take GapConnect");
wasm.ble_complete_gap_connect([0x11, 0x22, 0x33, 0x44, 0x55, 0x66]);
cpu.mem_write(0x20003ff0, [128, 0]);
check(svc(0x61, 0x20003000, 0x20003ff0) === 0, "mpy evt_get");
const evt = [...cpu.mem_read(0x20003000, 32)];
check(le16(evt, 0) === 0x10, "mpy CONNECTED id");
check(evt[4 + 16] === 2, "mpy CENTRAL role");
check(svc(0x96, 1, 0x13, 0) === 0, "mpy read stages");
wasm.ble_complete_gattc_read(1, 0x13, 0, [87]);
cpu.mem_write(0x20003ff0, [128, 0]);
check(svc(0x61, 0x20003000, 0x20003ff0) === 0, "mpy evt_get read");
const rr = [...cpu.mem_read(0x20003000, 17)];
check(le16(rr, 0) === 0x36 && rr[rr.length - 1] === 87, "mpy READ_RSP=87");
if (log.some((c) => !c)) process.exit(1);
console.log("mpy BLE face OK (MicroPython-idiom contract)");
